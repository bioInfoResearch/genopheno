...
class User:
    ...
    def __get_data_frame(self, snp_details, how):
        ...
        try:
            ...
            elif self.file_path.endswith("TRPV6.txt"):
                data_person = pd.read_table(self.file_path, header=0, comment="#", low_memory=False,
                                            error_bad_lines=False, delim_whitespace=True, warn_bad_lines=False)
                data_person = data_person.loc[:, data_person.columns.isin
                              (["Sample ID", "Allele1 - Forward", "Allele2 - Forward"])]
                data_person.columns = ["Rsid", "allele1", "allele2"]
                data_person["genotype"] = data_person["allele1"] + data_person["allele2"]
                data_person.drop(["allele2", "allele1"], axis=1, inplace=True)       
            else:
